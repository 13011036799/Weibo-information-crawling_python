...
with open('message.csv', "a", encoding="gbk", newline="") as f:
    csv_writer = csv.writer(f)
    csv_writer.writerow(['用户名',  '帖子内容','留言用户名',  '留言时间', '留言内容'])
    f.close()

with open('every_post.csv') as f:
    f=csv.DictReader(f)
    for row in f:
        web=row["具体微博网址"]
        print(web[2:47])
        ...
        user_name = driver.find_element_by_xpath('/html/body/div/div[1]/div[2]/div[2]/main/div[1]/div/div[2]/article/div[2]/header/div[1]/div/div[1]/a/span')
        content = driver.find_element_by_xpath('/html/body/div/div[1]/div[2]/div[2]/main/div[1]/div/div[2]/article/div[2]/div/div[1]/div')
